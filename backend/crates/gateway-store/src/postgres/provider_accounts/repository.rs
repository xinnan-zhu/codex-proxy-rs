...
impl ProviderAccountRepository for PgProviderAccountRepository {
    ...
    async fn list_plugin_accounts(
        ...
        let rows = sqlx::query(
            "select auto_location, detected_location_json, location_country, location_region, location_city, location_timezone, outbound_proxy_url, id, provider_kind, name, notes, codex_only, email, upstream_user_id,
...
pub(super) async fn import_provider_accounts_in_transaction(
    ...
    if let Some(settings) = &command.settings {
        ...
        if let Some(codex_only) = settings.codex_only {
            update_provider_account_codex_only_in_transaction(transaction, &unique_ids, codex_only)
                .await?;
        }
    }
...
pub(super) async fn rotate_provider_account_admin_in_transaction(
    ...
    if let Some(settings) = &command.settings {
        ...
        if let Some(codex_only) = settings.codex_only {
            update_provider_account_codex_only_in_transaction(transaction, ids, codex_only).await?;
        }
    }
