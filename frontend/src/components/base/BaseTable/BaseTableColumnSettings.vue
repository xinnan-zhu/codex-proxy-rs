...
const { option: setSortableOption } = useSortable(listRef, sortableOptions, {
  ...
  chosenClass: 'bg-cp-bg-text-hover',
    ...
    >
      ...
      >
        ...
        <BaseScrollbar max-height="min(20rem, calc(100dvh - 10rem))">
          <div ref="list" class="grid gap-0.5 p-2" role="list" aria-label="表格列">
            <div
              ...
              class="flex min-h-9 items-center rounded-cp pr-1"
              :class="!option.disabled || option.reorderable ? 'hover:bg-cp-bg-text-hover' : undefined"
            >
              <BaseCheckbox
