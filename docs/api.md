...
| `POST` | `/api/admin/accounts/update` | `{ accountId, enabled, concurrencyLimit, weight, groupIds, notes?, codexOnly?, modelAccess?, outboundProxyId?, outboundProxyUrl?, connection? }` | 一次更新账号设置；`connection` 支持 OpenAI OAuth 传输方式及 API Key 连接配置，见下文 |
...
收到 403 拒绝；管理端诊断（connection test）不受此限制。编辑与批量更新省略该字段时不修改；
导入 `settings` 提供时应用到本次导入的全部账号，省略时新账号默认关闭、已有账号保留原值。
...
省略不携带、保留账号已有值。重新授权不接受 `settings`，凭据刷新保留账号设置。
